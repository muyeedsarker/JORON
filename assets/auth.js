(() => {
  ...
  async function doLogin() {
    ...
    try {
      ...
      } else message("লগইন সম্পন্ন হয়নি। আবার চেষ্টা করুন।");
    } catch (error) {
      console.error("JORON login error", error);
      const code = error?.code || "";
      const text = ["auth/invalid-credential","auth/wrong-password","auth/user-not-found"].includes(code) ? "ইমেইল অথবা পাসওয়ার্ড সঠিক নয়." : code === "auth/invalid-email" ? "সঠিক ইমেইল ঠিকানা দিন." : code === "auth/too-many-requests" ? "অনেকবার চেষ্টা হয়েছে. কিছুক্ষণ পরে আবার চেষ্টা করুন." : code === "auth/network-request-failed" ? "ইন্টারনেট সংযোগ পরীক্ষা করুন." : `লগইন ব্যর্থ হয়েছে (${code || 'unknown-error'})`;
      message(text);
    } finally {
