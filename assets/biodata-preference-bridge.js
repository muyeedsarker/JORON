...
function addPreferencePanel(){
 ...
 pr.addEventListener("change",()=>{syncCommunity();syncHidden();writeOnboarding({partnerReligion:pr.value,partnerCommunity:pc.value})});syncCommunity();
 const gEl=document.getElementById("gender"),pg=document.getElementById("partnerGenderPref");
 gEl?.addEventListener("change",()=>{if(!pg.value)pg.value=gEl.value==="পুরুষ"?"নারী":gEl.value==="নারী"?"পুরুষ":"";syncHidden()});
