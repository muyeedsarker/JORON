...
// Existing Firebase/save logic is untouched. This file only controls the Address UI.
...
function makeSelect(id,labelText,ph,anchorId){if($(id))return $(id);const a=$(anchorId);if(!a)return null;const h=document.createElement("div");h.className="field joron-address-extra";h.innerHTML=`<label for="${id}">${labelText}</label><select id="${id}" name="${id}"><option value="">${ph}</option></select>`;a.closest(".field")?.insertAdjacentElement("beforebegin",h);return $(id);}
...
function setup(permanent){
 ...
 const dv=$(p+"division"),di=$(p+"district"),up=$(p+"upazila"),po=$(p+"postOffice"),pc=$(permanent?"ppostalCode":"postalCode"),area=$(permanent?"parea":"area");
 ...
 const village=makeSelect(p+"village",`${L} গ্রাম / মহল্লা`,`${L} গ্রাম নির্বাচন করুন`,p+"addressPrivacy");
 ...
 po.addEventListener("change",()=>{const row=P.find(x=>postLabel(x)===po.options[po.selectedIndex]?.textContent);if(pc)pc.value=row?code(row):"";summary(permanent);});
}
