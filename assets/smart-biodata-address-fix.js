// JORON Smart Biodata — Address FINAL v5
// Order: Division → District → Upazila/Thana → Union/Pouroshava → Ward → Post Office → Postal Code → Village/Mohalla → Area
// Works for both Permanent and Present. Legacy parea/area values are retained for the existing Firebase save system.
(()=>{
'use strict';
...
const bn=x=>String(x?.bn_name??x?.bnName??x?.name??'').trim();
const en=x=>String(x?.name??x?.en_name??'').trim();
const norm=s=>String(s??'').trim().toLowerCase();
const same=(a,b)=>norm(a)===norm(b)||norm(a)===norm(bn(b))||norm(a)===norm(en(b));
const fill=(s,rows,ph)=>{if(!s)return;s.replaceChildren(new Option(ph,''));const seen=new Set();for(const r of rows||[]){const n=bn(r)||en(r);const v=en(r)||n;if(n&&!seen.has(v)){seen.add(v);s.add(new Option(n,v))}}s.disabled=!seen.size};
...
function setup(pre){
 ...
 dv.onchange=()=>{reset(di,L+' জেলা নির্বাচন করুন');reset(up,L+' উপজেলা / থানা নির্বাচন করুন');reset(un,L+' ইউনিয়ন / পৌরসভা নির্বাচন করুন');reset(wd,L+' ওয়ার্ড নির্বাচন করুন');if(vl)vl.value='';if(ar)ar.value='';reset(po,L+' Post Office নির্বাচন করুন');if(pc)pc.value='';hidden(legacy?.id,'');const d=findDivision(dv.value);fill(di,d?.districts||[],L+' জেলা নির্বাচন করুন')};
 di.onchange=()=>{reset(up,L+' উপজেলা / থানা নির্বাচন করুন');reset(un,L+' ইউনিয়ন / পৌরসভা নির্বাচন করুন');reset(wd,L+' ওয়ার্ড নির্বাচন করুন');if(vl)vl.value='';if(ar)ar.value='';reset(po,L+' Post Office নির্বাচন করুন');if(pc)pc.value='';hidden(legacy?.id,'');const d=findDistrict(di.value);fill(up,d?.upazilas||[],L+' উপজেলা / থানা নির্বাচন করুন')};
 up.onchange=async()=>{reset(un,L+' ইউনিয়ন / পৌরসভা নির্বাচন করুন');reset(wd,L+' ওয়ার্ড নির্বাচন করুন');if(vl)vl.value='';if(ar)ar.value='';reset(po,L+' Post Office নির্বাচন করুন');if(pc)pc.value='';hidden(legacy?.id,'');const u=findUpazila(up.value);const list=[...(u?.unions||[]),...(u?.pourashavas||[])];fill(un,list,L+' ইউনিয়ন / পৌরসভা নির্বাচন করুন');wards(wd);const rows=P.filter(x=>{const n=String(x?.upazila??x?.upazila_name??x?.upazila_bn??'').trim();return same(n,u)});fill(po,rows.map(x=>({name:postName(x),bn_name:postName(x)})).filter(x=>x.name),L+' Post Office নির্বাচন করুন')};
...
function restore(pre){const legacy=$(pre?'parea':'area');if(!legacy?.value)return;const a=legacy.value.split(' | '),p=pre?'p':'',un=$(p+'union'),wd=$(p+'ward'),vl=$(p+'village'),ar=$(p+'areaText');if(un&&a[0]){const o=[...un.options].find(x=>norm(x.textContent)===norm(a[0]));if(o)un.value=o.value}if(wd&&a[1]){const n=a[1].match(/(\d+)/)?.[1];if(n)wd.value=n}if(vl)vl.value=a[2]||'';if(ar)ar.value=a.slice(3).join(' | ')||''}
async function get(u){const r=await fetch(u,{cache:'no-store'});if(!r.ok)throw Error('HTTP '+r.status);return r.json()}
async function init(){try{stat('📍 বাংলাদেশের ঠিকানার তালিকা লোড হচ্ছে...');const geo=await get(GEO);TREE=Array.isArray(geo)?geo:(geo?.divisions||geo?.data||[]);if(!TREE.length)throw Error('Bangladesh geo data empty');fill($('pdivision'),TREE,'Permanent বিভাগ নির্বাচন করুন');fill($('division'),TREE,'Present বিভাগ নির্বাচন করুন');setup(true);setup(false);try{const j=await get(POST);P=Array.isArray(j)?j:(j?.postcodes||j?.data||[])}catch(e){console.warn('Postcode source unavailable',e)}stat('✅ বিভাগ → জেলা → উপজেলা/থানা → ইউনিয়ন/পৌরসভা → ওয়ার্ড প্রস্তুত');setTimeout(()=>{restore(true);restore(false)},250)}catch(e){console.error('JORON address error:',e);stat('❌ ঠিকানার তালিকা লোড হয়নি। Internet চালু করে Refresh করুন।',true)}}
