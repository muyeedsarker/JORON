...
// Strict parent-child filtering: never falls back to another thana/district.
(()=>{
'use strict';
...
function postRows(d,di,t){
 const D=dist(d,di),T=thana(d,di,t); if(!D||!T)return [];
 const dn=norm(en(D)||bn(D)),tn=norm(en(T)||bn(T));
 return POSTS.filter(x=>{
   const xd=norm(x?.district??x?.district_name??x?.districtName??'');
   const xt=norm(x?.upazila??x?.upazila_name??x?.upazilaName??x?.thana??x?.thana_name??'');
   return xd===dn && xt===tn;
 });
}
function upId(d,di,t){const T=thana(d,di,t);return T?.id??T?.value??T?.code??T?.upazilaId??null;}
function areaRows(id,type){return (AREAS||[]).filter(a=>String(a.upazilaId??a.upazila_id)===String(id) && (!type||a.type===type));}
function villageRows(id){const unionIds=new Set(areaRows(id,'union').map(a=>String(a.id)));return (VILLAGES||[]).filter(v=>unionIds.has(String(v.areaId??v.area_id)));}
async function wardRows(d,di,t){const key=[d,di,t].map(norm).join('|');if(WC.has(key))return WC.get(key);const D=en(dist(d,di))||di,T=en(thana(d,di,t))||t;const where=`district='${String(D).replace(/'/g,"''")}' AND upazila='${String(T).replace(/'/g,"''")}'`;try{const r=await fetch(WARD+'?where='+encodeURIComponent(where)+'&outFields=division,district,upazila,union_,ward_name&returnGeometry=false&f=json',{cache:'no-store'});if(!r.ok)throw new Error('ward HTTP '+r.status);const j=await r.json();const rows=(j.features||[]).map(f=>f.attributes||f).filter(x=>x.ward_name);WC.set(key,rows);return rows;}catch(e){console.warn('JORON ward lookup',e);return [];}}
...
async function init(){try{msg('📍 Final Address Engine v18 লোড হচ্ছে...');let pd=$('pdivision');if(!pd&&$('pdistrict')){const b=$('pdistrict'),w=document.createElement('div');w.className='field';w.innerHTML='<label>Permanent বিভাগ *</label><select id="pdivision"><option value="">Permanent বিভাগ নির্বাচন করুন</option></select>';b.closest('.field').insertAdjacentElement('beforebegin',w);pd=$('pdivision');}
 ...
 fill($('division'),TREE,'Present বিভাগ নির্বাচন করুন');fill($('pdivision'),TREE,'Permanent বিভাগ নির্বাচন করুন');setup(false);setup(true);window.__JORON_ADDRESS_V18_READY=true;window.JORON_ADDRESS_READY=true;window.dispatchEvent(new CustomEvent('joronAddressReady'));msg('✅ Final Address v18: strict Division → District → Thana → Post Office → Ward → Village');
